...
import { GameState, PlayerStats, Enemy, Projectile, FloatingText, Upgrade, Particle, EnemyType } from './types';
import { INITIAL_UPGRADES, CANVAS_SIZE, COLOR_PALETTE, AVAILABLE_SKINS, MAX_ENEMIES_ON_SCREEN } from './constants';
import { Coins, Skull, Pause, Play, Zap, Radio, Heart, Trophy, User as UserIcon, PlayCircle, Shield, Diamond, Palette, X, Lock, Check, Calendar, Gift, Cloud, CloudLightning, LogOut, Mail, LockKeyhole, Tag, Copy, AlertTriangle, ShieldCheck, ArrowLeft, Save, Home, KeyRound } from 'lucide-react';
import { audioSystem } from './audio';
import { registerWithEmail, loginWithEmailOrUsername, logoutUser, saveGameToCloud, loadGameFromCloud, auth, CloudSaveData } from './firebase';
...
const App: React.FC = () => {
  ...
  const [authMode, setAuthMode] = useState<'hidden' | 'login' | 'register'>('login');
  ...
  // Daily Reward State
  const [dailyRewardPopup, setDailyRewardPopup] = useState<{ show: boolean, amount: number, streak: number } | null>(null);
  ...
  useEffect(() => {
    if (!currentUser || !gameState.isGameStarted || gameState.isGameOver) return;
  ...
  const handleCloudSave = async () => {
    ...
    const success = await saveGameToCloud(currentUser, gameState, upgrades, playerName, highScore);
    ...
    try {
        ...
        if (user) {
            ...
            if (data) {
                ...
                if (!isRegistering) {
                    const derivedName = user.displayName || user.email?.split('@')[0].toUpperCase() || cleanRegUser.toUpperCase();
        ...
        if (errCode.includes('permission-denied') || errMsg.includes('Missing or insufficient permissions')) {
            msg = "Erro de Segurança do Banco de Dados.";
            setShowRulesModal(true);
        }
        else if (errMsg === "USERNAME_TAKEN") msg = "Este nome de usuário já está em uso.";
        else if (errMsg === "USER_NOT_FOUND") msg = "Usuário não encontrado.";
        else if (errMsg === "USER_LOOKUP_FAILED") msg = "Falha ao buscar usuário. Tente com Email.";
        else if (errCode === 'auth/invalid-email') msg = "Email inválido.";
        else if (errCode === 'auth/user-not-found') msg = "Conta não encontrada.";
        else if (errCode === 'auth/wrong-password' || errCode === 'auth/invalid-credential') msg = "Senha incorreta.";
        else if (errCode === 'auth/email-already-in-use') msg = "Este email já possui conta.";
        else if (errCode === 'auth/weak-password') msg = "Senha muito fraca (min 6 caracteres).";
        else if (errCode === 'auth/unauthorized-domain') {
            msg = "Domínio não autorizado no Firebase.";
            const currentDomain = window.location.hostname;
            prompt("Copie este domínio e adicione no Firebase Authentication > Settings > Authorized Domains:", currentDomain);
        }
  ...
  const calculateStats = (currentUpgrades: Upgrade[], currentHp?: number, currentShield?: number): PlayerStats => {
     ...
     const health = currentHp !== undefined ? Math.min(currentHp, maxHealth) : maxHealth;
     
     const maxShield = s['max_shield'] || 0;
     const shield = currentShield !== undefined ? Math.min(currentShield, maxShield) : maxShield;
  ...
  const [stats, setStats] = useState<PlayerStats>(() => calculateStats(INITIAL_UPGRADES));
  ...
  const shockwaveRef = useRef<{active: boolean, radius: number, hitIds: Set<number>}>({ active: false, radius: 0, hitIds: new Set() });
  ...
  const spawnParticles = (x: number, y: number, color: string, count: number = 8) => {
    const effectiveCount = enemiesRef.current.length > 100 ? Math.ceil(count / 3) : count;
  ...
  const getSelectedSkin = () => {
      return AVAILABLE_SKINS.find(s => s.id === gameState.selectedSkinId) || AVAILABLE_SKINS[0];
  };
    ...
    const loop = (time: number) => {
      ...
      if (gameState.isGameOver) {
        if (gameState.score > highScore) {
           ...
           localStorage.setItem('neon_arena_highscore', gameState.score.toString());
           if (currentUser) handleCloudSave();
        }
        animationFrameId = requestAnimationFrame(loop);
        return;
      ...
      if (time - lastRegenTimeRef.current > REGEN_TICK_RATE / (gameState.gameSpeed || 1)) {
        setStats(prev => {
            ...
            if (newShield < prev.maxShield) {
                newShield = Math.min(prev.maxShield, newShield + (prev.maxShield * 0.05 * factor));
            }
            ...
            if (Math.floor(newShield) !== Math.floor(prev.shield) || Math.floor(newHp) !== Math.floor(prev.health)) {
                // Occasional text for regen
                if (prev.regen * factor >= 0.5 && Math.random() > 0.95 && newHp < prev.maxHealth) {
                    spawnFloatingText(CANVAS_SIZE/2, CANVAS_SIZE/2 - 40, `+${(prev.regen * factor).toFixed(1)}`, COLOR_PALETTE.success);
                }
      ...
      if (waveTimerRef.current >= WAVE_DURATION) {
        ...
        spawnFloatingText(CANVAS_SIZE/2, 100, `ONDA ${gameState.wave} COMPLETA!`, COLOR_PALETTE.accent);
        spawnFloatingText(CANVAS_SIZE/2, 130, `+$${waveBonus}`, COLOR_PALETTE.accent);
        spawnFloatingText(CANVAS_SIZE/2, 160, `+${gemBonus} GEMAS`, '#a855f7');
        audioSystem.playUpgrade();
      }
      
      setGameState(prev => ({ ...prev, waveProgress: (waveTimerRef.current / WAVE_DURATION) * 100 }));
        ...
        if (enemiesRef.current.length < MAX_ENEMIES_ON_SCREEN) {
            ...
            for(let i=0; i<spawnCount; i++) {
              ...
              enemiesRef.current.push({
                ...
                speed: (0.5 + Math.random() * 0.5) * (1 + (gameState.wave * 0.05)) * speedMult,
        ...
        if (enemy.stunTimer > 0) {
            ...
            if (dist > 30 + (enemy.type === 'boss' ? 20 : 0)) {
              ...
              if (hit) {
                ...
                setStats(prev => {
                  ...
                  if (currentShield > 0) {
                      if (currentShield >= rawDmg) {
                          ...
                          dmgToHp = 0;
                          spawnFloatingText(CANVAS_SIZE/2, CANVAS_SIZE/2, `ABSORVIDO`, COLOR_PALETTE.shield);
                      } else {
                          dmgToHp = rawDmg - currentShield;
                          currentShield = 0;
                          spawnFloatingText(CANVAS_SIZE/2, CANVAS_SIZE/2, `QUEBRA ESCUDO`, COLOR_PALETTE.shield);
                          addScreenShake(5); 
                          audioSystem.playHit();
                      }
                  }
                  ...
                  if (dmgToHp > 0) {
                    currentHp -= dmgToHp;
                    spawnFloatingText(CANVAS_SIZE/2, CANVAS_SIZE/2 + 20, `-${dmgToHp.toFixed(0)}`, '#ff0000');
                    addScreenShake(3);
                    if (currentHp <= 0) {
                ...
                spawnFloatingText(CANVAS_SIZE/2, CANVAS_SIZE/2, `ESQUIVA`, '#ffffff');
              }
      ...
      if (time - lastShotTimeRef.current > (1000 / currentStats.attackSpeed) / gameState.gameSpeed) {
        ...
        if (targetsToShoot.length > 0) {
          targetsToShoot.forEach(targetItem => {
              ...
              const dmg = currentStats.damage * (isCrit ? currentStats.critFactor : 1);
      ...
      projectilesRef.current.forEach(p => {
        ...
        if (target && !target.isDead) {
          ...
          if (distSq < hitDist * hitDist) {
            const distFromCenter = Math.sqrt(Math.pow(target.x - CANVAS_SIZE/2, 2) + Math.pow(target.y - CANVAS_SIZE/2, 2));
            const distanceBonus = 1 + ((distFromCenter / 10) * (currentStats.damagePerMeter / 100));
            ...
            if (!overloaded || p.isCrit || target.hp <= 0) {
                spawnFloatingText(target.x, target.y - 20, finalDamage.toFixed(0), p.isCrit ? COLOR_PALETTE.accent : '#fff');
            }
            ...
            if (target.hp <= 0) {
              ...
              if (target.type === 'speedster') { coinMult = 1.5; scoreMult = 1.5; }
              if (target.type === 'tank') { coinMult = 3; scoreMult = 3; }
              ...
              if (!overloaded) {
                  spawnFloatingText(target.x, target.y, `+$${coinReward.toFixed(0)}`, COLOR_PALETTE.accent);
              }
      ...
      if (shockwaveRef.current.active) {
        ...
        enemiesRef.current.forEach(e => {
            ...
            const dist = Math.sqrt(Math.pow(e.x - CANVAS_SIZE/2, 2) + Math.pow(e.y - CANVAS_SIZE/2, 2));
            ...
            if (Math.abs(dist - shockwaveRef.current.radius) < 20) {
                ...
                if (Math.random() > 0.8) spawnFloatingText(e.x, e.y, "STUN!", "#ffff00");
                ...
                if (e.hp <= 0) {
                    ...
                    spawnFloatingText(e.x, e.y, `+$${coinReward}`, COLOR_PALETTE.accent);
      ...
      textsRef.current.forEach(t => { t.y += t.vy; t.life--; });
  ...
  }, [gameState.isGameOver, gameState.gameSpeed, gameState.wave, gameState.isGameStarted, gameState.isPaused]);
  ...
  if (!gameState.isGameStarted) {
    return (
      <div className="flex flex-col h-[100dvh] w-full bg-gray-950 items-center justify-center relative overflow-hidden font-sans" onClick={handleInteraction}>
        ...
        <div className="z-10 bg-gray-900/90 p-8 rounded-2xl border border-cyan-500/50 shadow-[0_0_50px_rgba(0,255,255,0.15)] flex flex-col items-center max-w-sm w-full backdrop-blur-md relative overflow-hidden mx-4">
            ...
            <div className="mb-8 relative text-center">
                ...
                <p className="text-blue-200/60 text-xs uppercase tracking-wide font-share">Defesa do Núcleo v2.7 Secure</p>
            </div>
                    ...
                    <form onSubmit={handleAuthAction} className="flex flex-col gap-3">
                        <div className="flex bg-gray-950 border border-gray-700 rounded-md overflow-hidden mb-2">
                           <button 
                             ...
                             className={`flex-1 py-2 text-xs font-bold uppercase transition-colors ${authMode === 'login' ? 'bg-cyan-900 text-cyan-200' : 'text-gray-500 hover:text-white'}`}
                           >
                             Entrar
                           </button>
                           <button 
                             ...
                             className={`flex-1 py-2 text-xs font-bold uppercase transition-colors ${authMode === 'register' ? 'bg-cyan-900 text-cyan-200' : 'text-gray-500 hover:text-white'}`}
                           >
                             Criar
                           </button>
                        </div>
                        ...
                        <div className="space-y-2">
                             ...
                             <div className="relative">
                                ...
                                <input 
                                    ...
                                    placeholder={authMode === 'register' ? "Seu Email" : "Email ou Nome de Usuário"}
                                    value={authMode === 'register' ? regEmail : emailOrUser}
                                    onChange={e => authMode === 'register' ? setRegEmail(e.target.value) : setEmailOrUser(e.target.value)}
                             ...
                             <div className="relative">
                                <LockKeyhole className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={16} />
                                <input 
                                    type="password" 
                                    placeholder="Senha"
                                    value={password}
                        ...
                        >
                            {isSyncing ? <CloudLightning className="animate-pulse" size={14} /> : (authMode === 'register' ? 'Finalizar Cadastro' : 'Acessar Sistema')}
                        </button>
                    </form>
                 </div>
            ) : (
                     ...
                     <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-4 mb-6 flex flex-col gap-3 group relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 relative z-10">
                                ...
                                <div className="text-sm flex-1">
                                    <div className="text-green-500 text-[10px] uppercase font-bold tracking-wider">Operador Logado</div>
                                    <div className="font-bold text-white font-mono tracking-wide truncate">{tempName || currentUser?.displayName || "OPERADOR"}</div>
                                </div>
                        </div>
            ...
            <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/90 p-4">
                <div className="bg-gray-900 border border-green-500 rounded-xl max-w-lg w-full p-6 shadow-2xl relative">
                    ...
                    <div className="flex items-center gap-2 text-green-500 mb-4">
                        <ShieldCheck size={24} />
                        <h2 className="font-bold text-lg font-orbitron">SEGURANÇA DO SERVIDOR</h2>
                    </div>
                    
                    <p className="text-gray-300 text-sm mb-4">
                        Para garantir que <strong>nenhum jogador roube dados de outro</strong>, e permitir o login seguro, você deve atualizar as regras do banco de dados.
                    </p>
...
service cloud.firestore {
    ...
    match /usernames/{username} {
      ...
      allow create: if request.auth != null;
                    ...
                    <ol className="text-xs text-gray-400 list-decimal pl-5 space-y-1">
                        <li>Acesse o <a href="https://console.firebase.google.com/" target="_blank" className="text-blue-400 underline">Console Firebase</a> &gt; Firestore Database.</li>
                        <li>Clique na aba <strong>Rules</strong> (Regras).</li>
                        <li>Apague tudo e cole o código acima.</li>
                        <li>Clique em <strong>Publish</strong>. Sua segurança estará ativa.</li>
                    </ol>
                </div>
            </div>
        )}
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-gray-950 font-sans select-none overflow-hidden" onClick={handleInteraction}>
            ...
            <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md pointer-events-auto">
                <div className="bg-gray-900 border-2 border-green-500 p-8 rounded-2xl max-w-sm w-full text-center shadow-[0_0_50px_rgba(34,197,94,0.3)] transform scale-100 animate-[bounce_0.5s_ease-out]">
                    ...
                    <div className="text-green-400 mb-6 font-share text-lg">
                        Sequência: <span className="font-bold text-white">{dailyRewardPopup.streak} Dias</span>
                    </div>
                    ...
                    <div className="bg-black/50 p-6 rounded-xl mb-6 border border-green-900/50 flex flex-col items-center">
                        ...
                        <div className="text-4xl font-mono font-bold text-white mb-1">+{dailyRewardPopup.amount}</div>
                        <div className="text-xs text-gray-400 uppercase">Gemas Adicionadas</div>
                    </div>
      ...
      <div className="flex justify-between items-center px-2 md:px-4 py-2 bg-gray-900 border-b border-gray-800 z-10 shadow-lg relative h-12 md:h-16 shrink-0">
         <div className="flex gap-2 md:gap-4 items-center">
            ...
            {currentUser && (
                <div className={`
                    ...
                    ${cloudStatus === 'saving' ? 'border-yellow-500 text-yellow-500 animate-pulse' : 
                      cloudStatus === 'saved' ? 'border-green-500 text-green-500' :
                      cloudStatus === 'error' ? 'border-red-500 text-red-500' : 'border-gray-800 text-gray-600'}
                `}>
                    <Cloud size={12} />
                    {cloudStatus === 'saving' ? 'Salvando...' : 
                     cloudStatus === 'saved' ? 'Salvo' :
                     cloudStatus === 'error' ? 'Erro' : 'Cloud On'}
                </div>
            )}
         </div>
         ...
         <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center top-1 md:top-2">
          ...
          <div className="flex items-center gap-2">
             ...
             <button 
                onClick={togglePause}
                className={`p-1.5 md:p-2 rounded border transition-all active:scale-95 ${gameState.isPaused ? 'bg-yellow-900/40 border-yellow-500 text-yellow-400 animate-pulse' : 'bg-gray-800 border-gray-700 text-gray-400'}`}
             >
                {gameState.isPaused ? <Play size={16} className="md:w-5 md:h-5" /> : <Pause size={16} className="md:w-5 md:h-5" />}
             </button>
             <div className="text-right pl-2 border-l border-gray-800 hidden sm:block">
                <div className="text-[10px] text-gray-400 uppercase font-bold font-share">Nível</div>
                <div className="text-sm md:text-xl font-bold text-red-500 font-orbitron">ONDA {gameState.wave}</div>
             </div>
          </div>
      </div>
      ...
      <div className="flex-1 relative flex flex-col md:flex-row overflow-hidden">
          ...
          {gameState.isPaused && !showSkinModal && (
              <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] z-40 flex items-center justify-center pointer-events-auto">
                  <div className="flex flex-col items-center gap-4">
                    <h2 className="text-4xl font-black text-yellow-500 font-orbitron tracking-widest mb-2">PAUSADO</h2>
                    
                    <button onClick={togglePause} className="bg-yellow-600 hover:bg-yellow-500 text-black font-bold py-3 px-8 rounded-full shadow-[0_0_20px_rgba(234,179,8,0.5)] hover:scale-105 transition-transform flex items-center gap-2">
                        <Play size={24} fill="black" />
                        RETOMAR
                    </button>

                    <button onClick={returnToMainMenu} className="text-red-400 hover:text-red-300 font-bold uppercase text-xs tracking-wider flex items-center gap-2 border border-red-900/50 hover:bg-red-900/20 px-4 py-2 rounded transition-colors">
                        <LogOut size={14} />
                 ...
                 <div className="bg-gray-900 border border-purple-500/50 w-full max-w-4xl rounded-2xl shadow-[0_0_50px_rgba(168,85,247,0.2)] flex flex-col overflow-hidden max-h-[85vh]">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 scrollbar-thin scrollbar-thumb-purple-900">
                        {AVAILABLE_SKINS.map(skin => {
                            ...
                            return (
                                <div key={skin.id} className={`relative bg-gray-800/50 rounded-lg p-3 flex flex-col items-center border-2 transition-all group hover:bg-gray-800 ${isSelected ? 'border-green-500 bg-green-900/10' : isOwned ? 'border-gray-700' : 'border-gray-800 opacity-80'}`}>
                                    <div className="w-12 h-12 mb-3 flex items-center justify-center relative">
                                        ...
                                        <div style={{color: skin.color}} className="drop-shadow-[0_0_10px_currentColor] transform group-hover:scale-110 transition-transform duration-300">
                                            <svg width="32" height="32" viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="8">
                                                {skin.type === 'hexagon' && <path d="M50 5 L93 27 L93 72 L50 95 L7 72 L7 27 Z" />}
                                                {skin.type === 'triangle' && <path d="M50 10 L90 85 L10 85 Z" />}
                                                {skin.type === 'square' && <rect x="15" y="15" width="70" height="70" />}
                                                {skin.type === 'circle' && <circle cx="50" cy="50" r="40" />}
                                                {skin.type === 'star' && <path d="M50 10 L61 38 L91 38 L67 56 L76 84 L50 68 L24 84 L33 56 L9 38 L39 38 Z" />}
                                            </svg>
                                        </div>
                                    </div>
                                    
                                    <h3 className="font-orbitron text-[10px] text-white mb-2 text-center leading-tight h-6 flex items-center justify-center">{skin.name}</h3>
                                    
                                    {isOwned ? (
                                        <button 
                                            onClick={() => handleSkinAction(skin)}
                                            className={`w-full py-1.5 px-2 rounded text-[10px] font-bold font-mono flex items-center justify-center gap-1 ${isSelected ? 'bg-green-600 text-white cursor-default' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                                        >
                                            {isSelected ? <><Check size={12} /> EQUIPADO</> : 'EQUIPAR'}
                                        </button>
                                    ) : (
                                        <button 
                                            onClick={() => handleSkinAction(skin)}
                                            disabled={!canBuy}
                                            className={`w-full py-1.5 px-2 rounded text-[10px] font-bold font-mono flex items-center justify-center gap-1 ${canBuy ? 'bg-purple-600 hover:bg-purple-500 text-white' : 'bg-gray-800 text-gray-500 cursor-not-allowed'}`}
                                        >
                                            <Lock size={10} /> {skin.price > 1000 ? (skin.price/1000).toFixed(0) + 'K' : skin.price}
                                        </button>
                                    )}
                                </div>
                            );
              ...
              <div className="flex justify-between items-start pointer-events-auto w-full">
                  ...
                  <div className="flex bg-gray-900/90 rounded-lg p-1 border border-gray-700 backdrop-blur scale-90 origin-top-right md:scale-100">
                    {[1, 1.5, 2].map(speed => (
                        <button
                            key={speed}
                            onClick={() => setGameState(g => ({...g, gameSpeed: speed}))}
                            className={`px-2 md:px-3 py-1 rounded text-xs font-bold font-mono transition-colors ${gameState.gameSpeed === speed ? 'bg-cyan-700 text-white' : 'text-gray-500 hover:text-white'}`}
                        >
                            x{speed}
                        </button>
                    ))}
              ...
              {gameState.isGameOver && (
                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 pointer-events-auto">
                    <div className="bg-gray-900 border-2 border-red-600 p-8 rounded-2xl max-w-sm w-full text-center shadow-[0_0_50px_rgba(220,38,38,0.3)] transform scale-110">
                        <Skull size={48} className="mx-auto text-red-500 mb-4 animate-bounce" />
                        <h2 className="text-3xl font-black text-white mb-2 font-orbitron uppercase">Sistema Crítico</h2>
                        <p className="text-red-400 mb-6 font-share">O núcleo foi destruído.</p>
                        
                        <div className="bg-black/50 p-4 rounded-lg mb-6 border border-red-900/30">
                            <div className="text-xs text-gray-500 uppercase mb-1">Pontuação Final</div>
                            <div className="text-2xl font-mono font-bold text-yellow-400">{gameState.score.toLocaleString()}</div>
                        </div>
              ...
              <div className="flex flex-col items-center gap-2 md:gap-4 pointer-events-auto w-full max-w-2xl mx-auto mb-2">
                 ...
                 <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8 pointer-events-auto">
                     <button
                        ...
                        `}
                     >
                         <Zap size={20} className={`md:w-6 md:h-6 ${empCooldown === 0 && !gameState.isPaused ? "animate-pulse" : ""}`} />
                         {empCooldown > 0 && (
                            <div className="absolute inset-0 flex items-center justify-center bg-black/60 rounded-full">
                                <span className="text-[10px] md:text-xs font-bold text-white font-mono">{Math.ceil(empCooldown)}</span>
                            </div>
                         )}
                         <svg className="absolute inset-0 w-full h-full -rotate-90 pointer-events-none">
                            ...
                            {empCooldown > 0 && (
                                <circle 
                                    cx="50%" cy="50%" r="46%" fill="none" stroke="#00ffff" strokeWidth="2"
                                    strokeDasharray="289" // Approx circum for r=46
                                    strokeDashoffset={289 - (289 * (empCooldown / (stats.empCooldown || 30)))} 
        ...
        <div className="h-[40%] md:h-full md:w-96 bg-gray-900 border-t md:border-t-0 md:border-l border-gray-800 z-20 flex flex-col shadow-xl relative order-2">
             <div className="p-2 md:p-3 bg-gray-900 border-b border-gray-800 font-orbitron text-cyan-400 font-bold uppercase tracking-wider text-xs md:text-sm flex justify-between items-center shadow-md z-10 shrink-0">
                <div className="flex items-center gap-2">
                    <Radio size={14} className="animate-pulse text-cyan-500 md:w-4 md:h-4" />
                    <span>Engenharia</span>
                </div>
                <span className="text-[9px] md:text-[10px] text-gray-500 font-mono bg-black/30 px-2 py-0.5 rounded border border-gray-700">v2.5.0</span>
             </div>
             <UpgradePanel 
                upgrades={upgrades} 
                cash={gameState.cash} 
