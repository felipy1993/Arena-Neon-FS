...
import { PlayerStats, Enemy, Projectile, FloatingText, Particle, Skin } from '../types';
...
interface GameCanvasProps {
  ...
  shockwaveRef: React.MutableRefObject<{active: boolean, radius: number, hitIds: Set<number>}>;
  selectedSkin: Skin;
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (shockwaveRef.current.active) {
        ...
        ctx.lineWidth = 10;
        ctx.globalAlpha = Math.max(0, 1 - (shockwaveRef.current.radius / (CANVAS_SIZE/1.5)));
        ctx.stroke();
        ctx.restore();
      }
      ...
      enemies.current.forEach(enemy => {
        ...
        if (enemy.stunTimer > 0) {
            ...
            for(let i=0; i<segments; i++) {
                ...
                else ctx.lineTo(Math.cos(angle)*(r+jitter), Math.sin(angle)*(r+jitter));
            }
            ctx.closePath();
            ctx.stroke();
            ctx.restore();
      ...
      // Draw Projectiles
      if (!lowQualityMode) ctx.shadowBlur = 10;
      projectiles.current.forEach(proj => {
        if (!lowQualityMode) ctx.shadowColor = proj.color;
        ...
        if (!lowQualityMode) {
            ...
            ctx.lineTo(proj.x - (proj.speed * 2) * Math.cos(angle), proj.y - (proj.speed * 2) * Math.sin(angle));
            ctx.stroke();
        }
      });
      ctx.shadowBlur = 0;
      ...
      if (currentStats.maxShield > 0) {
        const shieldPct = Math.max(0, currentStats.shield / currentStats.maxShield);
        if (shieldPct > 0) {
            ctx.beginPath();
            ctx.arc(0, 0, 38, -Math.PI/2, (-Math.PI/2) + (Math.PI * 2 * shieldPct));
      ...
      // HEALTH RING (INNER)
      const healthPct = Math.max(0, currentStats.health / currentStats.maxHealth);
      ctx.beginPath();
      ...
      texts.current.forEach(t => {
        if (lowQualityMode && !t.text.includes('$') && !t.text.includes('!')) return;
